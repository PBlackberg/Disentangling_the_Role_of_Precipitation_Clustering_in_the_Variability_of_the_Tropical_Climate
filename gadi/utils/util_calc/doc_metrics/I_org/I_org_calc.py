...
def get_observed_cdf(N_c, lat_coords, lon_coords, r_bin_edges):
    ...
    return obs_cdf, cumulative_sum


# == main ==
def main(da, lat_coords, lon_coords, r_bin_edges, dx):
    # -- density of cores --
    ...
    # -- only calculate with enough cores --
    if N_c < 2:
        obs_cdf = np.ones_like(random_cdf) * np.nan
        i_org = np.nan
        cumulative_sum = np.ones_like(r_bin_edges) * np.nan
    else:
        # -- obs cdf --
        obs_cdf, cumulative_sum = get_observed_cdf(N_c, lat_coords, lon_coords, r_bin_edges)
        # -- area under obs cdf as a function of random cdf --
        i_org = np.trapz(obs_cdf, random_cdf)
    return i_org, obs_cdf, random_cdf, cumulative_sum, N_c
