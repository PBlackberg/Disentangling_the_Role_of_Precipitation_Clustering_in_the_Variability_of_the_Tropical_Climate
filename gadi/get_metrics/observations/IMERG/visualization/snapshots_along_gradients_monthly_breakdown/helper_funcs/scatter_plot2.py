...
def plot_a_scatter(x, y, fig, ax, month, r_bin_lim, ii, idx_t):
    ...
    # phase-space bins
    # xbins = np.linspace(-2.5, 3.5, 10)
    # ybins = np.linspace(-2.5, 5.5, 10)
    # xbins = np.percentile(x0, np.arange(0, 101, 10))
    # ybins = np.percentile(y0, np.arange(0, 101, 20))

    # xbins = np.percentile(x0, np.arange(0, 101, 5))
    # ybins = np.percentile(y0, np.arange(0, 101, 5))

    xbins = np.percentile(x0, np.arange(0, 101, 10))
    ybins = np.percentile(y0, np.arange(0, 101, 10))

    # -- bins centres in the middle of box
    # xc = (xbins[:-1] + xbins[1:]) / 2
    # yc = (ybins[:-1] + ybins[1:]) / 2

    # U = np.full((len(yc), len(xc)), np.nan)
    ...
    #         mask = (
    #             (x0 >= xbins[i]) & (x0 < xbins[i+1]) &
    #             (y0 >= ybins[j]) & (y0 < ybins[j+1])
    #         )

    #         if np.sum(mask) >= 5:
    #             U[j, i] = np.mean(dx[mask])
    #             V[j, i] = np.mean(dy[mask])
    ...
    # -- bins centres in the middle of distribution in box
    X = np.full((len(ybins)-1, len(xbins)-1), np.nan)
    ...
    for i in range(len(xbins)-1):
        for j in range(len(ybins)-1):
            ...
            if np.sum(mask) >= 5:
                # representative position of observations
                # X[j, i] = np.mean(x0[mask])
                # Y[j, i] = np.mean(y0[mask])
                X[j, i] = np.median(x0[mask])
                Y[j, i] = np.median(y0[mask])
    ...
    for edge in ybins:
        ax.axhline(edge, color='purple', lw=0.3, alpha=0.5)
